...
```http
GET /api/worlds/:worldId/events?page=1&limit=20
```
...
| `firstVersion.readEntities[].schemaId` | `number` | Yes | Entity schema ID to read. |
...
```http
GET /api/worlds/:worldId/events/:eventName/event-versions?page=1&limit=20
```

### Create Event Version

```http
POST /api/worlds/:worldId/events/:eventName/event-versions
```
...
```json title="Request Body"
{
  ...
  "readEntities": [{ "schemaId": 53, "filter": ["eq:instanceId:HORSE_1"] }],
  "stateChanges": { "horse": "partial" },
...
| `readEntities[].schemaId` | `number` | Yes | Entity schema ID to read. |
...
```http
POST /api/worlds/:worldId/events/:eventName/event-versions/:version/publish
```
