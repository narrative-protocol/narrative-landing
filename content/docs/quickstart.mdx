...
- `stateChanges`: Per-schema mutation mode (`partial`, `full`, `append`). `partial` merges into existing state, `full` replaces state, and `append` appends to arrays.
- `outputSchema`: Defines the expected public result fields.
- `behaviorPrompt`: Prompt used by AI mode.
